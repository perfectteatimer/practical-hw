...
from model_l import LanguageModel
...
def training_epoch(model: LanguageModel, optimizer: torch.optim.Optimizer, criterion: nn.Module,
    ...
    for indices, lengths in tqdm(loader, desc=tqdm_desc):
        ...
        indices = indices[:, :maxlen] # obez ba4 do maks len
        input_sequence = indices[:, :-1] # vse krome last
        target = indices[:, 1:maxlen].to(device) # vse so 2go
        new_len = lengths - 1 # mins 1 kazd raz dlinu
        optimizer.zero_grad()
        # forward
        logits = model(input_sequence, new_len) # (batch_size, L, vocab_size) L = maxdlina - 1
        ...
        train_loss += loss.item() * input_sequence.size(0) # loss * #els in ba4 for avg 
...
def validation_epoch(model: LanguageModel, criterion: nn.Module,
    ...
    for indices, lengths in tqdm(loader, desc=tqdm_desc):
        ...
        logits = model(input_sequence, new_len) # (batch_size, L, vocab_size) L = maxdlina - 1
        loss = criterion(logits.reshape(-1, model.vocab_size), target.reshape(-1))
        val_loss += loss.item() * input_sequence.size(0) # loss * #els in ba4 for avg 
